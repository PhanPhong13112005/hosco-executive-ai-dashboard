...
export function AlertsPage() {
  ...
  const open = async (id: string) => {
    ...
    try { setDetail(await api.alert(id)); setResolutionNote('') }
  ...
  const transition = async (action: 'acknowledge' | 'resolve') => {
    if (!detail) return
    setActionBusy(true)
    try { setDetail(action === 'acknowledge' ? await api.acknowledge(detail.id) : await api.resolve(detail.id, resolutionNote)); await load() }
  ...
  return <>
    ...
    <section className="filter-bar compact">
      ...
      <label>Mức độ<select value={filter.severity} onChange={e => setFilter({ ...filter, severity: e.target.value })}><option value="">Tất cả</option><option>Medium</option><option>High</option><option>Critical</option></select></label>
    ...
    {(detailLoading || detail) && <div className="drawer-backdrop" onClick={() => setDetail(null)}><aside className="detail-drawer" onClick={e => e.stopPropagation()}><button className="modal-close" onClick={() => setDetail(null)}>×</button>{detailLoading ? <StatePanel kind="loading" title="Đang tải Alert" message="Đang kiểm tra phạm vi truy cập."/> : detail && <><div className="detail-title"><span className={`severity ${detail.severity.toLowerCase()}`}>{detail.severity}</span><span>{detail.ruleCode}</span></div><h2>{detail.title}</h2><p>{detail.message}</p><dl><dt>Trạng thái</dt><dd>{detail.status}</dd><dt>Chi nhánh</dt><dd>{detail.branchId ?? 'Scope đã cấp'}</dd><dt>Giá trị phát hiện</dt><dd>{detail.detectedValue ?? '—'}</dd><dt>Baseline</dt><dd>{detail.baselineValue ?? '—'}</dd><dt>Ngưỡng cấu hình</dt><dd>{detail.thresholdValue ?? '—'}</dd><dt>Thời điểm</dt><dd>{new Date(detail.detectedAt).toLocaleString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })} (UTC+7)</dd>{detail.resolutionNote && <><dt>Ghi chú xử lý</dt><dd>{detail.resolutionNote}</dd></>}</dl><details><summary>Chi tiết kỹ thuật / detection context</summary><p><strong>Dedup key:</strong> <code>{detail.dedupKey}</code></p><pre>{prettyJson(detail.contextJson)}</pre></details>{detail.status !== 'Resolved' && <label>Ghi chú xử lý {(detail.ruleCode === 'AL-04' || detail.ruleCode === 'AL-05') && '(bắt buộc)'}<textarea maxLength={2000} value={resolutionNote} onChange={e => setResolutionNote(e.target.value)} placeholder="Mô tả nguyên nhân và cách xử lý…"/></label>}<div className="drawer-actions">{detail.status === 'Open' && <button disabled={actionBusy} className="button secondary" onClick={() => void transition('acknowledge')}>Đánh dấu đã xem</button>}{detail.status !== 'Resolved' && <button disabled={actionBusy || ((detail.ruleCode === 'AL-04' || detail.ruleCode === 'AL-05') && !resolutionNote.trim())} className="button primary" onClick={() => void transition('resolve')}>Đánh dấu đã xử lý</button>}</div></>}</aside></div>}
  </>
}
