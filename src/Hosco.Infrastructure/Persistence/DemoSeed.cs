...
public static class DemoSeed
{
    ...
    public static async Task SeedAsync(HoscoDbContext db, CancellationToken ct = default)
    {
        ...
        foreach (var tenant in tenants)
        {
            ...
            {
                Id = DemoSeedIds.Id($"{tenant.Code}-product-{i}"), TenantId = tenant.Id, Sku = $"{tenant.Code}-SKU-{i:000}",
                Name = $"Demo Product {i}", CurrentPrice = 25_000m + i * 7_500m, CurrentCost = 15_000m + i * 4_000m,
                Currency = "VND", CreatedAt = now, UpdatedAt = now
            }).ToArray();
            db.Products.AddRange(products);
            var customers = Enumerable.Range(1, 16).Select(i => new Customer
            {
                Id = DemoSeedIds.Id($"{tenant.Code}-customer-{i}"), TenantId = tenant.Id, Name = $"Customer {i}",
                Email = $"customer{i}@{tenant.Code.ToLowerInvariant()}.local", PhoneMasked = $"***{i:0000}", CreatedAt = now, UpdatedAt = now
            }).ToArray();
            db.Customers.AddRange(customers);
            var employees = tenantBranches.SelectMany((branch, bi) => Enumerable.Range(1, 3).Select(i => new Employee
            {
                Id = DemoSeedIds.Id($"{branch.Code}-employee-{i}"), TenantId = tenant.Id, BranchId = branch.Id,
                EmployeeCode = $"{branch.Code}-E{i:00}", DisplayName = $"Cashier {bi + 1}-{i}", CreatedAt = now, UpdatedAt = now
            })).ToArray();
            ...
            foreach (var (product, index) in products.Select((p, i) => (p, i)))
                ...
                {
                    Id = DemoSeedIds.Id($"inventory-{branch.Id}-{product.Id}"), TenantId = tenant.Id, BranchId = branch.Id,
                    ProductId = product.Id, QuantityOnHand = index < 2 && branch.Id == tenantBranches[0].Id ? 2 + index : 35 + index,
                    SafetyStock = 8, CreatedAt = now, UpdatedAt = new DateTimeOffset(2026, 6, 30, 18, 0, 0, TimeSpan.Zero)
                });
            ...
            foreach (var branch in tenantBranches)
            {
                ...
                for (var n = 0; n < count; n++)
                {
                    ...
                    var order = new Order
                    {
                        Id = orderId, TenantId = tenant.Id, BranchId = branch.Id, CustomerId = customers[sequence % customers.Length].Id,
                        EmployeeId = employee.Id, OrderNumber = $"{tenant.Code}-{sequence:000000}", Status = status, OrderedAt = orderedAt,
                        Subtotal = subtotal, DiscountAmount = discount, TotalAmount = total, Currency = "VND", CreatedAt = orderedAt, UpdatedAt = orderedAt
                    };
                    db.Orders.Add(order); db.OrderItems.AddRange(item1, item2);
                    db.Payments.Add(new Payment
                    {
                        Id = DemoSeedIds.Id($"payment-{orderId}"), TenantId = tenant.Id, OrderId = orderId,
                        Method = (PaymentMethod)(sequence % 4), Status = status == OrderStatus.Cancelled ? PaymentStatus.Refunded : PaymentStatus.Paid,
                        Amount = total, PaidAt = orderedAt.AddMinutes(2), CreatedAt = orderedAt, UpdatedAt = orderedAt
                    });
                    if (status == OrderStatus.Returned)
                        db.Refunds.Add(new Refund
                        {
                            Id = DemoSeedIds.Id($"refund-{orderId}"), TenantId = tenant.Id, BranchId = branch.Id, OrderId = orderId,
                            Status = RefundStatus.Completed, Amount = total, ReasonCode = "DEMO_RETURN", RequestedAt = orderedAt.AddDays(2),
                            CompletedAt = orderedAt.AddDays(3), CreatedAt = orderedAt.AddDays(2), UpdatedAt = orderedAt.AddDays(3)
                        });
                }
            }
        }
    ...
    {
        Id = DemoSeedIds.Id($"item-{orderId}-{index}"), TenantId = tenantId, OrderId = orderId, ProductId = product.Id,
        Quantity = quantity, UnitPrice = product.CurrentPrice, UnitCostAtSale = product.CurrentCost, DiscountAmount = discount,
        LineTotal = product.CurrentPrice * quantity - discount, CreatedAt = now, UpdatedAt = now
    };
    private static void AddAnomaly(HoscoDbContext db, Guid tenantId, Guid branchId, string type, string title, DateTimeOffset at) =>
        ...
        {
            Id = DemoSeedIds.Id($"alert-{tenantId}-{type}"), TenantId = tenantId, BranchId = branchId, Type = type,
            Severity = AlertSeverity.Warning, Status = AlertStatus.Open, Title = title,
            PayloadJson = JsonSerializer.Serialize(new { fixture = true, expectedWindowStart = at }), DetectedAt = at,
